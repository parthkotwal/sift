...
2. The public repo contains **code and architecture** — that's ours. Dataset-derived *metrics* (recall/NDCG scores, holdout size, hit attribution) **are** published in the README and `DECISIONS.md`, as an author's decision under the terms' academic-purposes reading: Sift is an educational build, and the metrics are inseparable from the engineering claims they justify — "rerank costs 42% of recall@10" is the finding, and a version without the number is not the same statement. Where a doc needs scale context, official public figures (150K businesses, ~7M reviews) are used since Yelp publishes those itself.

   **This is a judgment call, recorded as one.** An earlier version of this rule said to be conservative and keep result-y numbers in local notes, while the README published them — the repo contradicted itself, which is worse than either choice. The line now drawn: *aggregate* metrics over the frozen holdout are published; **raw records, review text, real user or business IDs, and derived artifacts are never committed**, and `data/` stays gitignored forever. That boundary is what rules 1 and 3 protect, and it is not affected by this decision.

   Anyone reusing this repo should make their own determination rather than inherit this one, and the bundled `Dataset_User_Agreement.pdf` in the TAR is the authority — not this summary. Note that the agreement version is itself uncertain: this file cites a July 2023 version, and the bundled PDF has been read elsewhere as February 16, 2021. Its text could not be extracted with the tooling here (subset-encoded fonts), so **the version above is unverified**. It does not change rule 1 or the boundary in rule 2, since every version cited restricts metrics disclosure and both readings leave the raw-data prohibitions identical.
