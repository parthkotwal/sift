...
**Not yet servable.** `ONLINE_STATE_GROUPS` excludes the ALS slices, so `online_features()` omits `ui_als_score` and the Redis path still serves the eight-feature model. Deploying this ranker requires publishing current ALS state first (ISSUES.md I25); training may use the full registry, but shipping a model that reads a feature serving cannot supply is skew by construction.
